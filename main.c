...
void __interrupt() isr (void){
    if(PIR1bits.ADIF){                      // Fue interrupci?n del ADC?
        if(ADCON0bits.CHS == 0){            // Verificamos sea AN0 el canal seleccionado
        ...
        else{                                   // Verificamos sea AN0 el canal seleccionado
            ...
            CCP2CONbits.DC2B0 = CCPR_2 & 0b11; // Guardamos los 2 bits menos significativos en DC1B
        }
...
void main(void) {
    ...
    while(1){
        if(ADCON0bits.GO == 0b00){                 
            if(ADCON0bits.CHS == 0b00){   
                ADCON0bits.CHS = 0b01;    // Cambio de canal
            }
            else if(ADCON0bits.CHS == 0b01){
            ADCON0bits.CHS = 0b00;
            }
                    // Cambio de canal
 ...
 * CONFIGURACION 
 ------------------------------------------------------------------------------*/
void setup(void){
    ANSEL = 0xff;                // AN0 como entrada anal?gica
    ANSELH = 0;                 // I/O digitales
    TRISA = 0xff;                // AN0 como entrada
    PORTA = 0;
    
    // Configuraci?n reloj interno
    OSCCONbits.IRCF = 0b011;    // 4MHz
